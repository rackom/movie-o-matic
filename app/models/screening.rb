...
class Screening < ActiveRecord::Base
  def self.get_percent_filling_by_movie_id(movieID)
    ...
    #MySQL
    find_by_sql(["select cast(floor(time_before_screening/1) as decimal(7,1)) as tbs, avg(seats_taken/seats_total) as obsadenost from screenings where movie_id = ? group by tbs;", movieID])
  end
end